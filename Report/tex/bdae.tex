...
\section{Libraries}
BDAE provides three python libraries one for each of the user tier described in section \ref{sec:user-tiers}: \texttt{libbdaescientist}, \texttt{libbdaemanager} and \texttt{libbdaeadmin} respectively, that implements the logic listed.
...
A simple graphical user interface represented as a website (illustrated in figure \ref{fig:website}) is provided as part of the BDAE installation (explained in appendix \ref{app:installation}). 
...
The Android application provides a handheld modern access to the scientist level of BDAE, implemented in a minimalist Google approved material design \cite{PageMaterialDesign} as visualized in figure \ref{fig:app-overview} and appendix \ref{chp:app-screenshots}.
\newline
...
\begin{itemize}
	...
	\item Listing meta data information about all datasets currently available in \CodeName.
