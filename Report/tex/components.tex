...
Each configuration specification is written in a \texttt{.cfg} file format (Definition \ref{def:cfg})) with four predefined groups:
\begin{itemize}
...
Although the strategies initially appear to be approximately equal, the results of appending eight blocks on three storage nodes are very divergent (Figure \ref{fig:distribution-strategies}) and thus one could easily find very different purposes for each:
...
\end{enumerate}
\vspace*{2mm}
Though, there is a common limitation for all three of them; data \textbf{overflow} from first to last when transferring left and last to first when transferring right are not implemented for the time being.
\newline
